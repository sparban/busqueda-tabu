...
nDim = 7                            # numero de dimensiones de la funsion  (20, 50, 100)
rango = [-100, 100]
...
def aplicar_tweck (s,tw):
    ...
    # Aqui debo decirle que las probabilidades lista tabu las remplze por cero

    print("lista de probabilidades: ",listaProbabillidad)
    for i in range(len(s)):
        # Conversion probabilidades
        if listaProbabillidad[i] > 0.2:
...
# Funcion para aplicar tweak
def aplicar_tweck2(s,tw,list1):
    vc2 = np.reshape(np.array(list1), (-1))
    # lista carcateristicas Modificadas
    LCM = list(vc2)
    print("Lista Caracteristicas Modificadas: ",LCM)
    print(len(LCM))
    Generaltwick = []
    listaProbabillidad = funcion_probabilidad(s)
    print("lista de probabilidades: ",listaProbabillidad)
    ...
    for i in range(len(LCM)):
        indice = LCM[i]
        print(listaProbabillidad[indice])
        listaProbabillidad[indice]=0
    
    print("lista de probabilidades Cambiadas: ",listaProbabillidad)
...
for i in range(0,3):
    ...
    if iteracionActual > 0:
        #for i in range(len(listaCaracteristicas)):
                ...
                #print("Entra al primer ciclo")
        
        
        # Encontramos el valor de R
        print("Ciclo - Iteracion: ",iteracionActual)
        iteracionActual += 1
        tw_R = genracionTW (s)
        print(f"Tw de R: {tw_R}")
        R=aplicar_tweck2(s, tw_R,listaComparacion)
        print("vector R", R)
        ...
        for i in range(0,n-1):
            ...
            W = aplicar_tweck2(s, tw_W,listaComparacion)
            print(f"Vector W: {W}")
        ...
        var1 = np.reshape(np.array(ListaTupla[0]), (-1))
        listaComparacion.append(list(var1))
        ...
        if QS < QBest:
            ...
            QBest = QS
        print("Calidad de best", QBest)
        print("Lita caracteristicas", listaCaracteristicas)
        print("Lista Comapracion: ",listaComparacion)
